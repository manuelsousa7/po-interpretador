...
/**
 * Class DepositAccount is an abstract class representing a deposit account.
 * Subclasses refine this class in accordance with the type of service provided
 * by the bank. In its most abstract form, a deposit account keeps track of the
 * account's balance, interest rate, deposit date and a deposit number. This
 * number uniquely identifies deposit accounts.
 * 
...
public abstract class DepositAccount implements Serializable {
	...
	public DepositAccount(double rate, double amount) {
		...
		_id = _nextId++;
		_rate = rate;
		_initialDate = Calendario.obtemCalendario().hoje();
	}

	/**
	 * Deposit a given amount in the deposit account.
	 * 
	 * @param amount
	 *            the amount to be deposited.
	 * @throws InvalidDeposit
	 *             if it not possible to perform the depois.
	 */
	abstract public void deposit(double amount) throws InvalidDeposit;

	/**
	 * Withdraw a given amount from the deposit account.
	 * 
	 * @param amount
	 ...
	 */
	public abstract void withdraw(double amount) throws InvalidWithdrawal;

	/**
	 * Add to the current balance (protected!).
	 * 
	 * @param the amount to increase (or decrease).
	 */
	 ...
	 * 
	 * @param depacc
	 *            the other deposit account.
	 * @return true, if both objects have the same number; false, otherwise.
	 */
	@Override
	public final boolean equals(Object depacc) {
		return depacc instanceof DepositAccount
				&& ((DepositAccount) depacc).getId() == _id;
	}
